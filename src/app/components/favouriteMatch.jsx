...
function FavMatchCard({ data }) {
  const { removefromFavourites } = useMatchContext();
        ...
        <div className='   bg-white rounded-[5px] shadow-md'>
          ...
          {data.matchStarted && !data.matchEnded &&
            <div className='p-2 flex justify-between text-[14px] '>
              ...
              <button onClick={(e) => { e.preventDefault(); removefromFavourites(data.id) }} className='text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-lime-300 rounded-lg px-2 py-1 text-center me-2'>Remove from fav</button>
            </div>
          }
          {/* if match ended */}
          {data.matchEnded &&
